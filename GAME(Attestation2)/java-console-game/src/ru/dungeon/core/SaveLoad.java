...
public class SaveLoad {
    ...
    public static void load(GameState s) {
        ...
        try (BufferedReader r = Files.newBufferedReader(SAVE)) {
            ...
            Room currentRoom = findRoomByName(map.getOrDefault("room", "Пещера"), s.getCurrent());
    ...
    private static Room findRoomByName(String roomName, Room startingRoom) {
        if (startingRoom == null || roomName == null || roomName.isEmpty()) return null;
        ...
        while (!queue.isEmpty()) {
            ...
            for (Room neighbor : room.getNeighbors().values()) queue.add(neighbor);
        }
        return null;
    }
